...
class DeliveryRequestBuilderTests: XCTestCase {
    ...
    func testGetDisplayNotification_withNoProfileState() {
        ...
        XCTAssertEqual("iOS 14.2", notification?.parameters?["a.OSVersion"])
        XCTAssertEqual(1, notification?.profileParameters?.count)
        XCTAssertEqual("pval1", notification?.parameters?["profileParam1"])
        XCTAssertNotNil(notification?.order)
        XCTAssertEqual("pval1", notification?.order?.id)
